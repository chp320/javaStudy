...
public class HashMapMethod {
    public static void main(String[] args) {
        ...
        hMap2.replace(1, "가가가", "나나나"); // (1, "가가가") 인 쌍의 값을 "나나나" 로 변경 -> O
        hMap2.replace(2, "다다다", "라라라"); // (2, "다다다") 인 쌍의 값을 "라라라" 로 변경 -> x
        System.out.println(hMap2.toString());
        ...
        // 8. Set<K> keySet()
        Set<Integer> keySet = hMap2.keySet();   // 현재 저장된 엔트리 중 key 값만을 추출하여 Set<E>로 가져옴
        System.out.println(keySet.toString());

        // 9. Set<Map.Entry<K,V>> entrySet()
        Set<Map.Entry<Integer, String>> entrySet = hMap2.entrySet();    // 엔트리에서 (key, value) 쌍을 Set<E> 로 가져옴
